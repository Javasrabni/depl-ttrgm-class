...
function collides() {
    return currentShape.some((row, rowIndex) => {
        return row.some((cell, colIndex) => {
            if (cell) {
                let newX = currentX + colIndex;
                let newY = currentY + rowIndex;
...
function mergeShape() {
    currentShape.forEach((row, rowIndex) => {
        row.forEach((cell, colIndex) => {
            if (cell) {
                board[currentY + rowIndex][currentX + colIndex] = currentShapeColor;
            }
        });
    });
    checkForLines();
...
function startGame() {
    ...
    resetShape();
    gameInterval = setInterval(gameLoop, 325); // Set game speed
...
function rotateShape() {
    const rotatedShape = currentShape[0].map((_, i) =>
        currentShape[0].map(row => row[i]).reverse()
    );

    // Save the current shape and position
    ...
    // Check if the rotated shape collides
    if (collides()) {
        currentShape = tempShape;
        currentX = tempX;
        currentY = tempY;
    } else {
        // Adjust position if out of bounds horizontally
        while (collides()) {
            currentX -= 1;
            if (!collides()) return;
            currentX += 2;
